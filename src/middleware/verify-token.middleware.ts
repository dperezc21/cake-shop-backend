...
export class VerifyTokenMiddleware {
    async verifyUserToken(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            if(!token) {
                ResponseUtil.responseJson(res, "No token provided", false, 401);
                return ;
            }
            jwt.verify(token, SECRET_JSON_WEB_TOKEN, async(err: VerifyErrors, result) =>{
               if(err) ResponseUtil.responseJson(res, "token did not authenticated", false, 401);
                else {
                   ...
                   if(!user?.dataValues?.id) ResponseUtil.responseJson(res, "user not authenticated", false, 401);
                   else next();
               }
            });
        } catch(err) {
